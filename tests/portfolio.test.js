...
describe('calculateOwnerTotal', () => {
  ...
  const accounts = [
    { owner: 'Alice', asset: 'bitcoin', amount: 1, type: 'crypto' },
    { owner: 'Alice', asset: 'ethereum', amount: 2, type: 'crypto' },
    { owner: 'Bob', asset: 'bitcoin', amount: 0.5, type: 'crypto' },
  ]

  it('sums value for the correct owner only', () => {
    // Alice: 1 BTC * 50000 + 2 ETH * 3000 = 56000
    expect(calculateOwnerTotal(accounts, prices, 'Alice')).toBe(56000)
  })

  it('handles owner with a single asset', () => {
    // Bob: 0.5 BTC * 50000 = 25000
    expect(calculateOwnerTotal(accounts, prices, 'Bob')).toBe(25000)
  })

  it('returns 0 for unknown owner', () => {
    expect(calculateOwnerTotal(accounts, prices, 'Charlie')).toBe(0)
  })

  it('falls back to 0 when price is missing', () => {
    const noPrices = {}
    expect(calculateOwnerTotal(accounts, noPrices, 'Alice')).toBe(0)
  })
})
...
describe('generateCSV', () => {
  ...
  it('includes the correct headers', () => {
    const csv = generateCSV(accounts, prices)
  ...
  it('includes one data row per account', () => {
    const csv = generateCSV(accounts, prices)
  ...
  it('escapes notes containing commas', () => {
    const withComma = [{ ...accounts[0], notes: 'bought, then sold' }]
...
describe('parseCSV', () => {
  it('round-trips accounts through generateCSV → parseCSV', () => {
    const prices = { bitcoin: { usd: 50000 } }
    const original = [
      {
        owner: 'Alice',
    ...
    expect(parsed[0].owner).toBe('Alice')
    expect(parsed[0].asset).toBe('bitcoin')
  ...
  it('filters out rows with missing required fields', () => {
    const malformed = 'Owner,Type,Asset,Amount\n,,,'
